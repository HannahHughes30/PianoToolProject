...
class SheetMusicProcessor:
    ...
    def load_image(self, image_path):
        """Load image with error handling, supports PDF, PNG, JPG"""
    ...
    def auto_rotate_image(self, image):
        """Automatically rotate image to portrait orientation if needed"""
        h, w = image.shape[:2]
        if w > h:
    ...
    def preprocess_pipeline(self, image_path, output_dir='data/processed'):
        ...
        if self.debug_mode:
            cv2.imwrite(f'{output_dir}/{base_name}_rotated.png', img)
            cv2.imwrite(f'{output_dir}/{base_name}_binary.png', binary)
            viz = img.copy()
            for y in staff_lines:
                cv2.line(viz, (0, y), (viz.shape[1], y), (0, 255, 0), 3)
            cv2.imwrite(f'{output_dir}/{base_name}_detected.png', viz)
            print(f"\n  💾 Saved output files to: {output_dir}/")
        return {'binary_image': binary, 'staff_lines': staff_lines, 'bar_lines': bar_lines, 'original': img}
